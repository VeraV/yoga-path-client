...
export function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (profileData) {
          const recData = await recommendationApi
            .getLatest(profileData.id)
            .catch(() => null);
  ...
  // Calculate stats
  const totalMinutesThisWeek = recentLogs.reduce(
    (sum, log) => sum + log.minutesPracticed,
    0,
  );
        ...
        <section className="dashboard-card">
          ...
          {recentLogs.length > 0 ? (
            <div className="card-content">
              ...
              <p className="card-detail">
                Last:{" "}
                {new Date(recentLogs[0].practiceDate).toLocaleDateString()}
              </p>
            </div>
          ) : (
            <div className="card-content card-empty">
